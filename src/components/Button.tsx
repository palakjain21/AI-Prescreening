...
const getVariantColorClasses = (variant: ButtonVariant = "solid", color: ButtonColor = "primary") => {
  const colorMap = {
    primary: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "solid", color = "primary", size = "default", asChild = false, count, children, ...props }, ref) => {
    ...
    if (asChild && React.isValidElement(children)) {
      ...
      return React.cloneElement(children, {
        className: cn(baseClasses, getVariantColorClasses(variant, color), getSizeClasses(size), className, childProps.className),
        ref,
        ...props,
      } as any);
    }
    ...
    return (
      <button
        className={cn(baseClasses, getVariantColorClasses(variant, color), getSizeClasses(size), className)}
        ref={ref}
        {...props}
      >
        {children}
        {count !== undefined && (
          <span className={cn(
            "ml-2 rounded-md px-2 py-0.5 text-xs font-regular",
            variant === "solid"
              ? "bg-white/20 text-white disabled:bg-neutral-300 disabled:text-neutral-400" 
              : "bg-neutral-100 text-neutral-500 disabled:bg-neutral-200 disabled:text-neutral-400"
          )}>
            {count}
          </span>
        )}
